neoxr.create(async (m, {
   ...
   try {
      ...
      if (command == 'buyall') {
         ...
         if (users.limitGame >= maximum) return client.reply(m.chat, Func.texted('bold', `🚩 Sorry, you can't buy any more game limits because you have reached maximum limit.`), m)
         if (isNaN(args[0])) return client.reply(m.chat, Func.example(prefix, command, '1'), m)
         if (args[0] < 1) return client.reply(m.chat, Func.example(prefix, command, '1'), m)
         if (users.point >= price * parseInt(args[0])) {
            if ((users.limitGame + parseInt(args[0])) >= maximum) return client.reply(m.chat, Func.texted('bold', `🚩 Game Limit amount you buy exceeds maximum limit.`), m)
            users.point -= price * parseInt(args[0])
